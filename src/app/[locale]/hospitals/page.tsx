...
import {
  Search, SlidersHorizontal, X, Star, Shield, Crown,
  Building2, MapPin, RotateCcw, Filter, Phone, Globe,
  Stethoscope
} from 'lucide-react';
...
import type { Hospital, City, Doctor } from '@/types';
...
export default function HospitalsPage() {
  ...
  useEffect(() => {
    initializeStore();
    const allHospitals = getAll<Hospital>('hospitals');
  ...
  const resetFilters = useCallback(() => {
    setFilters({ search: '', city: '', sort: 'rank' });
  }, []);
  ...
  const filteredHospitals = useMemo(() => {
    ...
    if (filters.search) {
      ...
      result = result.filter(h =>
        h.name.toLowerCase().includes(q) || h.name_ar.includes(q)
      );
    }

    if (filters.city) {
      result = result.filter(h => h.city_id === filters.city);
    }
  ...
  const renderFilters = () => (
    <div className="space-y-5">
      <div>
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder={t('home.search_placeholder_hospital')}
      ...
      <div>
        ...
        >
          <option value="rank">{locale === 'ar' ? 'الأعلى رتبة' : 'Highest Rank'}</option>
          <option value="rating">{locale === 'ar' ? 'الأعلى تقييمًا' : 'Highest Rated'}</option>
        </select>
      </div>
  ...
  return (
    <div className="container-custom py-6 md:py-8">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-gray-900">{t('hospital.title')}</h1>
          <p className="text-gray-500 mt-1">
            {locale === 'ar' ? `عرض ${filteredHospitals.length} من ${hospitals.length} مستشفى` : `Showing ${filteredHospitals.length} of ${hospitals.length} hospitals`}
          </p>
        </div>
        <button
          ...
          {filteredHospitals.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
              {filteredHospitals.map((hospital) => {
                const cityName = getCityName(hospital.city_id);
                const RankIcon = hospital.rank !== 'normal' ? rankIcons[hospital.rank] : null;
                const logoColor = getLogoColor(hospital.id);
                const displayName = locale === 'ar' && hospital.name_ar ? hospital.name_ar : hospital.name;
                const departments = hospital.departments?.split('，').filter(Boolean).slice(0, 3) || [];

                return (
                  <Link key={hospital.id} href={`/hospitals/${hospital.id}`} className="card block cursor-pointer hover:shadow-lg hover:-translate-y-0.5">
                    <div className="flex items-start gap-4 mb-4">
                      <div className={cn('w-14 h-14 rounded-full flex items-center justify-center text-lg font-bold flex-shrink-0', logoColor)}>
                        {hospital.logo_url ? (
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {RankIcon && (
                            <RankIcon className={cn('w-4 h-4 flex-shrink-0 fill-current', rankIconColors[hospital.rank])} />
                          )}
                        </div>
                    ...
                    {departments.length > 0 && (
                      <div className="flex flex-wrap gap-1.5 mb-3">
                        {departments.map((dept, i) => (
                          <span key={i} className="px-2 py-0.5 bg-gray-100 text-gray-600 rounded text-xs">{dept}</span>
                        ))}
                        {hospital.departments && hospital.departments.split('，').length > 3 && (
                          <span className="px-2 py-0.5 text-gray-400 text-xs">
                            +{hospital.departments.split('，').length - 3}
                          </span>
                        )}
                      </div>
                    )}
